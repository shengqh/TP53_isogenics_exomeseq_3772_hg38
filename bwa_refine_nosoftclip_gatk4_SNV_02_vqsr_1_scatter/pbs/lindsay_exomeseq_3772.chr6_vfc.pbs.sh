...
if [ ! -s lindsay_exomeseq_3772.chr6.merged.vcf.gz ]; then
  echo CombineGVCFs=`date` 
  gatk --java-options "-Xmx40G" \
    CombineGVCFs \
    -R /data/cqs/references/broad/hg38/v0/bwa_index_0.7.17/Homo_sapiens_assembly38.fasta  -L chr6 \
      ...
      -O lindsay_exomeseq_3772.chr6.merged.vcf.gz
fi

if [[ -s lindsay_exomeseq_3772.chr6.merged.vcf.gz && ! -s lindsay_exomeseq_3772.chr6.raw.vcf.gz ]]; then
  ...
  gatk --java-options "-Xmx40G" \
    ...
    -O lindsay_exomeseq_3772.chr6.raw.vcf.gz \
    -V lindsay_exomeseq_3772.chr6.merged.vcf.gz 
fi

if [[ -s lindsay_exomeseq_3772.chr6.raw.vcf.gz && ! -s lindsay_exomeseq_3772.chr6.variant_filtered.vcf.gz ]]; then
  echo VariantFiltration=`date` 
  gatk --java-options "-Xmx40G" \
    VariantFiltration \
    --filter-expression "ExcessHet > 54.69" \
    --filter-name ExcessHet \
    -O lindsay_exomeseq_3772.chr6.variant_filtered.vcf.gz \
    -V lindsay_exomeseq_3772.chr6.raw.vcf.gz
fi

if [[ -s lindsay_exomeseq_3772.chr6.variant_filtered.vcf.gz && ! -s lindsay_exomeseq_3772.chr6.variant_filtered.sites_only.vcf.gz ]]; then
  echo MakeSitesOnlyVcf=`date` 
  gatk --java-options "-Xmx40G" \
    MakeSitesOnlyVcf \
    --INPUT lindsay_exomeseq_3772.chr6.variant_filtered.vcf.gz \
    --OUTPUT lindsay_exomeseq_3772.chr6.variant_filtered.sites_only.vcf.gz
fi

if [[ -s lindsay_exomeseq_3772.chr6.variant_filtered.sites_only.vcf.gz ]]; then
  rm lindsay_exomeseq_3772.chr6.merged.vcf.gz lindsay_exomeseq_3772.chr6.merged.vcf.gz.tbi \
    lindsay_exomeseq_3772.chr6.raw.vcf.gz lindsay_exomeseq_3772.chr6.raw.vcf.gz.tbi .conda
fi
